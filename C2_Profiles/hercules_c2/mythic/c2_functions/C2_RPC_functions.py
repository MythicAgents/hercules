...
async def test(request):
    ...
    resp = await MythicRPC().execute(
        "create_event_message", message="Test message", warning=False
    )
    return response
...
async def opsec(request):
    ...
    if "https" in params["callback_host"] and params["callback_port"] not in [
        ...
        return {
            "status": "error",
            "error": f"Mismatch - HTTPS specified, but port {params['callback_port']}, is not one of the standard port (443, 8443)\n",
        }
        ...
        with open("../c2_code/config.json") as f:
            ...
            for inst in config["instances"]:
                ...
                if str(inst["port"]) == str(request["parameters"]["callback_port"]):
                    if (
                        "https" in request["parameters"]["callback_host"]
                        ...
                        "https" not in request["parameters"]["callback_host"]
                        and inst["use_ssl"]
                    ):
