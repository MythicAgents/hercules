...
async def translate_to_c2_format(request) -> bytes:
    if request["mythic_encrypts"]:
        ...
        # print(request["message"])
        jsonBytes = bytearray(json.dumps(request["message"]).encode())
        ...
        return base64.b64encode(
            request["uuid"].encode() + json.dumps(request["message"]).encode()
        )
